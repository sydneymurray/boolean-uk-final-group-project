-- CreateEnum
CREATE TYPE "Format" AS ENUM ('DUBPLATE', 'MP3', 'VINYL6', 'VINAL7', 'VINAL10', 'VINAL12');

-- CreateTable
CREATE TABLE "Track" (
    "id" SERIAL NOT NULL,
    "artistName" VARCHAR(255) NOT NULL,
    "trackName" VARCHAR(255) NOT NULL,
    "coverURL" VARCHAR(255) NOT NULL,

    PRIMARY KEY ("id")
);

-- CreateTable
CREATE TABLE "Listings" (
    "id" SERIAL NOT NULL,
    "user" INTEGER NOT NULL,
    "track" INTEGER,
    "album" INTEGER,
    "price" DOUBLE PRECISION NOT NULL,
    "forSale" BOOLEAN NOT NULL DEFAULT true,
    "notes" VARCHAR(255) NOT NULL,
    "condition" VARCHAR(255) NOT NULL,
    "format" "Format" NOT NULL DEFAULT E'MP3',
    "dateAdded" TIMESTAMP(3) NOT NULL DEFAULT CURRENT_TIMESTAMP,

    PRIMARY KEY ("id")
);

-- CreateTable
CREATE TABLE "Album" (
    "id" SERIAL NOT NULL,
    "artist" TEXT NOT NULL,
    "albumname" TEXT NOT NULL,
    "coverURL" TEXT NOT NULL,

    PRIMARY KEY ("id")
);

-- AddForeignKey
ALTER TABLE "Listings" ADD FOREIGN KEY ("user") REFERENCES "User"("id") ON DELETE CASCADE ON UPDATE CASCADE;

-- AddForeignKey
ALTER TABLE "Listings" ADD FOREIGN KEY ("track") REFERENCES "Track"("id") ON DELETE SET NULL ON UPDATE CASCADE;

-- AddForeignKey
ALTER TABLE "Listings" ADD FOREIGN KEY ("album") REFERENCES "Album"("id") ON DELETE SET NULL ON UPDATE CASCADE;
